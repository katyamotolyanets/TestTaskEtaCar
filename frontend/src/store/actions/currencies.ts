...
export const addToCurrencies = (id: string, name: string, price: string, changePercentDay: string) => {
    return async (dispatch: Dispatch<CurrencyAction>) => {
        ...
        dispatch({type: CurrenciesActionTypes.ADD_TO_CURRENCIES, payload: currency})
    }
}

export const fetchWalletCurrenciesData = (topCurrencies: CurrencyType[], currencies: CurrencyType[]) => {
    return async (dispatch: Dispatch<CurrencyAction>) => {
        try {
...
export const fetchCurrentCurrencies = (data: CurrencyType[]) => {
    return async (dispatch: Dispatch<CurrencyAction>) => {
        try {
            dispatch({type: CurrenciesActionTypes.FETCH_CURRENT_CURRENCIES_SUCCESS, payload: data})
        } catch (error) {
            dispatch({type: CurrenciesActionTypes.FETCH_CURRENCIES_FAILED, payload: 'Cannot get data'})
        }
    }
